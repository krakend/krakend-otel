...
func newTransportMetrics(metricsOpts *TransportMetricsOptions, meter metric.Meter) *transportMetrics {
	...
	tm.requestsStarted, _ = meter.Int64Counter("requests-started-count")   // number of reqs started
	tm.requestsFailed, _ = meter.Int64Counter("requests-failed-count")     // number of reqs failed
	tm.requestsCanceled, _ = meter.Int64Counter("requests-canceled-count") // number of canceled requests
	...
	tm.responseLatency, _ = meter.Float64Histogram("response-latency")
	tm.responseContentLength, _ = meter.Int64Histogram("response-content-length")

	tm.detailsEnabled = metricsOpts.DetailedConnection
	tm.getConnLatency, _ = meter.Float64Histogram("request-get-conn-latency")
	tm.dnsLatency, _ = meter.Float64Histogram("request-dns-latency")
	tm.tlsLatency, _ = meter.Float64Histogram("request-tls-latency")
	return &tm
}
