...
//
// TODO: we might want to pass a logger, to report issues when trying to
...
func newInstruments(prefix string,
	...
	// TODO: check if implementations of meter and tracer, when they
	// fail, return a noop "whatever" instead of a nil reference
	nopMProvider := noopmetric.NewMeterProvider()
	nopM := nopMProvider.Meter(fmt.Sprintf("%s-nop-tracker", prefix))

	sizeCount, _ := nopM.Int64Counter(strSizeCount)
	sizeHistogram, _ := nopM.Int64Histogram(strSizeHistogram)
	timeCount, _ := nopM.Float64Counter(strTimeCount)
	timeHistogram, _ := nopM.Float64Histogram(strTimeHistogram)
	...
	if meter != nil {
		...
		if bsh, err := meter.Int64Histogram(strSizeHistogram); err == nil {
			sizeHistogram = bsh
		}
		if brtc, err := meter.Float64Counter(strTimeCount, metric.WithUnit("s")); err == nil {
			timeCount = brtc
		}
		if brth, err := meter.Float64Histogram(strTimeHistogram, metric.WithUnit("s")); err == nil {
			timeHistogram = brth
		}
	...
	if tracer == nil {
		tracer = nooptrace.NewTracerProvider().Tracer("readed-tracer")
	}
